...
def check_exceptions(f):
    def wrapper(*args, **kwargs):
        try:
            ...
            app.log.error(code=ex.code, class_name='helpers', method_name='check_exceptions',
...
def build_error_response(error_code, message, response, status_code):
    return {
...
def log_request(f):
    def wrapper(*args, **kwargs):
        ...
        app.log.info(class_name='helpers', method_name='log_request', message='Request recebido: ' + message)
...
def get_git_repo():
    ...
    git_path = git_path.replace("/app/helpers", "").replace("\\app\\helpers","")
    repo = Repo(git_path)
    return repo
